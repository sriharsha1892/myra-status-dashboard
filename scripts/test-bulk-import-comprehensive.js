/**
 * Comprehensive Bulk Import Test
 * Tests the AI bulk import with a realistic narrative containing ~10 events
 */
...
async function testBulkImport() {
  ...
  try {
    ...
    console.log(`✅ Testing with: ${orgs.org_name} (${orgs.org_id})`);

    // Step 2: Get initial event count
    console.log('\n📋 Step 2: Checking initial event count...');
    const { data: initialEvents, error: countError } = await supabase
    ...
    console.log('\n📋 Step 3: Creating authenticated session...');
    const { data: authData, error: authError } = await supabase.auth.signInWithPassword({
      email: 'admin@myra.ai',
      password: 'admin@myRA2024'
    });
    ...
    // Step 4: Test the LLM parser
    console.log('\n📋 Step 4: Testing LLM parser with comprehensive narrative...');
    console.log(`📝 Narrative length: ${TEST_NARRATIVE.length} characters`);
    console.log('⏱️  Parsing with Groq (expecting <2 seconds)...\n');

    const startTime = Date.now();

    const response = await fetch('http://localhost:3000/api/timeline/import/llm-parse', {
      method: 'POST',
      headers: {
    ...
    if (result.events && result.events.length > 0) {
      ...
      // Step 5: Store test data IDs for cleanup
      console.log('\n📋 Step 5: Simulating import to database...');
      console.log('(This will create actual records that we\'ll clean up)');
      ...
      console.log(`✅ Imported ${testEventIds.length} test events to database`);
      ...
      const finalCount = finalEvents?.length || 0;
      console.log(`✅ Database now has: ${finalCount} events (was ${initialCount})`);
      console.log(`✅ Test created: ${finalCount - initialCount} new events`);
      ...
      if (deleteError) {
        ...
        console.log(`⚠️  Manual cleanup needed for IDs: ${testEventIds.join(', ')}`);
      } else {
        console.log(`✅ Cleaned up ${testEventIds.length} test events`);
      }

      // Final verification
      const { data: cleanedEvents } = await supabase
        .from('trial_timeline_events')
        .select('id')
        .eq('org_id', orgs.org_id);

      const cleanedCount = cleanedEvents?.length || 0;
      console.log(`✅ Database restored to: ${cleanedCount} events`);

      // Final summary
      console.log('\n' + '='.repeat(80));
      console.log('\n🎉 TEST SUMMARY:\n');
      console.log(`✅ Parsing works: YES (${parseTime}ms)`);
      console.log(`✅ Events extracted: ${result.events.length} (expected ~10)`);
      console.log(`✅ Average confidence: ${Math.round(avgConfidence * 100)}%`);
      console.log(`✅ Import works: YES`);
      console.log(`✅ Cleanup works: YES`);
      console.log(`✅ Performance: ${parseTime < 3000 ? 'EXCELLENT' : parseTime < 5000 ? 'GOOD' : 'NEEDS IMPROVEMENT'}`);
