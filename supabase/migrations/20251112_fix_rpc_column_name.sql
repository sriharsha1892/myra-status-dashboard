-- Fix RPC function to use correct column name (full_name instead of name)
...
BEGIN
  ...
  -- Get actor's name for notification (FIXED: use full_name instead of name)
  SELECT full_name INTO v_actor_name FROM users WHERE id = v_user_id;
