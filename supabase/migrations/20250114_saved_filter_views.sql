...
-- Create indexes
CREATE INDEX idx_saved_views_user ON roadmap_saved_views(user_id);
CREATE INDEX idx_saved_views_org ON roadmap_saved_views(org_id);
CREATE INDEX idx_saved_views_default ON roadmap_saved_views(user_id, is_default) WHERE is_default = true;
CREATE INDEX idx_saved_views_shared ON roadmap_saved_views(org_id, is_shared) WHERE is_shared = true;
CREATE INDEX idx_saved_views_quick ON roadmap_saved_views(user_id, quick_access) WHERE quick_access = true;
...
CREATE INDEX idx_user_prefs_user ON roadmap_user_preferences(user_id);
CREATE INDEX idx_user_prefs_org ON roadmap_user_preferences(org_id);
...
CREATE INDEX idx_filter_history_user ON roadmap_filter_history(user_id, applied_at DESC);
...
-- =====================================================
-- 4. CREATE SHARED VIEW ACCESS TABLE
-- =====================================================
...
CREATE INDEX idx_view_access_view ON roadmap_view_access(view_id);
CREATE INDEX idx_view_access_user ON roadmap_view_access(user_id);
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE view_id = p_view_id
          AND (user_id = v_user_id OR team_id IN (
            SELECT team_id FROM team_members WHERE user_id = v_user_id
          ))
      )
    );
