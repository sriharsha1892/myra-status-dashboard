...
CREATE INDEX IF NOT EXISTS idx_feature_request_vote_comments_vote_id ON feature_request_vote_comments(vote_id);
...
-- Drop existing views if they exist (they may have different column structure)
DROP VIEW IF EXISTS feature_requests_with_votes CASCADE;
DROP VIEW IF EXISTS roadmap_items_with_votes CASCADE;

-- View to get feature requests with user vote status
CREATE VIEW feature_requests_with_votes AS
SELECT
  fr.*,
  CASE
...
-- Create indexes with IF NOT EXISTS
CREATE INDEX IF NOT EXISTS idx_roadmap_item_votes_roadmap_id ON roadmap_item_votes(roadmap_id);
CREATE INDEX IF NOT EXISTS idx_roadmap_item_votes_user_id ON roadmap_item_votes(user_id);
CREATE INDEX IF NOT EXISTS idx_roadmap_vote_comments_vote_id ON roadmap_vote_comments(vote_id);
...
CREATE VIEW roadmap_items_with_votes AS
SELECT
  r.*,
  CASE
...
DROP POLICY IF EXISTS "Users can view all feature request votes" ON feature_request_votes;
CREATE POLICY "Users can view all feature request votes"
  ON feature_request_votes FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "Users can manage their own votes" ON feature_request_votes;
...
DROP POLICY IF EXISTS "Users can view all vote comments" ON feature_request_vote_comments;
CREATE POLICY "Users can view all vote comments"
  ON feature_request_vote_comments FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "Users can manage their own vote comments" ON feature_request_vote_comments;
...
DROP POLICY IF EXISTS "Users can view all roadmap votes" ON roadmap_item_votes;
CREATE POLICY "Users can view all roadmap votes"
  ON roadmap_item_votes FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "Users can manage their own roadmap votes" ON roadmap_item_votes;
...
DROP POLICY IF EXISTS "Users can view all roadmap vote comments" ON roadmap_vote_comments;
CREATE POLICY "Users can view all roadmap vote comments"
  ON roadmap_vote_comments FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "Users can manage their own roadmap vote comments" ON roadmap_vote_comments;
...
DO $$
BEGIN
  RAISE NOTICE '✅ Comprehensive voting system successfully created!';
  RAISE NOTICE '';
  RAISE NOTICE 'Features:';
  RAISE NOTICE '  1. Feature request voting with comments';
  RAISE NOTICE '  2. Roadmap item voting with comments';
  RAISE NOTICE '  3. Enhanced reactions (helpful, solved)';
  RAISE NOTICE '  4. Vote notifications';
  RAISE NOTICE '  5. Milestone notifications';
END $$;
