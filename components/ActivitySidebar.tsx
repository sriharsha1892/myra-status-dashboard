...
export default function ActivitySidebar({ isOpen, onClose }: ActivitySidebarProps) {
  ...
  const getNotificationColor = (type: Notification['notification_type']) => {
    const colors = {
      ...
      comment: 'text-purple-600 bg-purple-50 dark:bg-purple-900/20',
      ...
      watching_update: 'text-slate-600 bg-slate-50 dark:bg-slate-900/20'
    };
    return colors[type];
  };
  ...
  return (
    <div className="fixed right-0 top-0 h-screen w-96 bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-700 shadow-xl z-50 flex flex-col">
      {/* Header */}
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      ...
                      {notification.status === 'unread' && (
                        <button
                          ...
                          className="text-xs px-2 py-1 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded hover:bg-slate-300 dark:hover:bg-slate-600 flex items-center gap-1"
                        >
