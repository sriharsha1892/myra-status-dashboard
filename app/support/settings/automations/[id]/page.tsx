...
export default function EditAutomationPage({ params }: PageProps) {
  ...
  const fetchRule = async () => {
    try {
      ...
      const rule: AutomationRule = data.data;
      setName(rule.name);
      setDescription(rule.description || '');
      setEntityType(rule.entity_type);
      setTriggerType(rule.trigger_type);
      setTriggerEvent(rule.trigger_event);
      setConditions(rule.conditions as ConditionGroup);
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        schedule_cron: triggerType === 'schedule' ? '*/15 * * * *' : null,
        conditions,
        actions,
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900">
      ...
      <header className="h-14 bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 px-6 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Edit Automation</h2>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
