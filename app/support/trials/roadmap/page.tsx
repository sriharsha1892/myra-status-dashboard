...
export default function RoadmapPage() {
  ...
  const fetchOrganizations = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        const savedOrgId = typeof window !== 'undefined' ? localStorage.getItem('selectedRoadmapOrgId') : null;
        ...
        setSelectedOrg(selected || (data as any)[0]);

        // Save to localStorage
        if (typeof window !== 'undefined') {
          localStorage.setItem('selectedRoadmapOrgId', defaultOrgId);
        }
      }
    } catch (error: any) {
  ...
  const handleOrgChange = (newOrgId: string) => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('selectedRoadmapOrgId', newOrgId);
    }
  ...
  const handleBackToOrg = () => {
    if (orgId) {
      router.push(`/support/trials/${orgId}?tab=Roadmap`);
    }
  };
  ...
  return (
    <div>
      <main className="flex-1 overflow-y-auto">
        ...
        <header className="bg-white/80 backdrop-blur-xl border-b border-gray-200/60 shadow-sm sticky top-0 z-10">
          <div className="px-8 py-4 flex items-center justify-between border-b border-gray-200/40">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">Manage and track product roadmap items</p>
            </div>
            {orgId && (
              <button
        ...
        <div className="p-8">
          {orgId ? (
            <ProductRoadmapTabEnhanced orgId={orgId} />
          ) : (
