...
export default function FeaturesPage() {
  ...
  const fetchOrganizations = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        const orgParam = typeof window !== 'undefined' ? searchParams?.get('org') : null;
        const savedOrgId = typeof window !== 'undefined' ? localStorage.getItem('selectedFeaturesOrgId') : null;
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('selectedFeaturesOrgId', defaultOrgId);
        }
      }
    } catch (error: any) {
  ...
  const handleOrgChange = (newOrgId: string) => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('selectedFeaturesOrgId', newOrgId);
    }
  ...
  const handleBackToOrg = () => {
    if (orgId) {
      router.push(`/support/trials/${orgId}?tab=Features`);
    }
  };
    ...
    <main className="flex-1 overflow-y-auto">
        ...
        <header className="bg-white/80 backdrop-blur-xl border-b border-gray-200/60 shadow-sm sticky top-0 z-10">
          <div className="px-8 py-4 flex items-center justify-between border-b border-gray-200/40">
            <div>
              <h2 className="text-xl font-bold text-gray-900 tracking-tight">Feature Requests</h2>
              <p className="text-xs text-gray-500 mt-0.5">Track and manage feature requests across trials</p>
            </div>
            {orgId && (
              <button
