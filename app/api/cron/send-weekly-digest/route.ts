 ...
 *
 * This endpoint should be called once per week by a cron service (e.g., Vercel Cron, GitHub Actions, etc.)
 * It generates and sends weekly digest emails to all users who have opted in.
 *
 * Security: Protected by CRON_SECRET environment variable
 *
 * Example cron schedule: 0 9 * * 1 (9 AM every Monday)
 *
 * Usage:
 * curl -X POST https://your-domain.com/api/cron/send-weekly-digest \
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await generateAndSendDigests('weekly');
    ...
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Failed to process weekly digests',
      },
      { status: 500 }
    );
  }
